...
public class Tubi : IMediaSource
{
	static Tubi()
	{
		Tubi.CategoryNamesByInitialSourceURL = new Dictionary<string, string>
		{
			{ "https://tubitv.com/", string.Empty },
			{ "https://tubitv.com/movies", string.Empty },
			{ "https://tubitv.com/tv-shows", string.Empty },
			{ "https://tubitv.com/kids", string.Empty },
			{ "https://tubitv.com/categories", string.Empty }
		};

		Tubi.CategoryNamesBySourceURL = new Dictionary<string, string>();
	}
	...
	public static async Task SetCategoryNamesBySourceURLAsync()
	{
		...
		ProgressBarManager.IncrementMaximum(Tubi.CategoryNamesByInitialSourceURL.Count, mediaSourceName);

		using var httpClient = new HttpClient();
		foreach (var (uRL, category) in Tubi.CategoryNamesByInitialSourceURL)
		{
			Tubi.CategoryNamesBySourceURL.Add(uRL, category);
			...
			const string indicatorCategoryBlock = "href=\"/category/";
			...
			foreach (var startPositionCategoryBlock in startPositions)
			{
				...
				name = Tubi.GetFixedCategoryName(name);

				Tubi.CategoryNamesBySourceURL.TryAdd("https://tubitv.com/" + urlSuffix, name);
			}
	...
	public static async Task<HashSet<string>?> GetSearchResultsAndAddNewMediaToViewModelAsync(string sourceURL, string categoryName)
	{
		...
		if (ViewModel.MoreSpeedFewerPosters && !sourceURL.Contains("https://tubitv.com/search/"))
		{
			using var httpClient = new HttpClient();
		...
		{
			var title = Tubi.GetTitle(pageHtml, startPositionMediaBlock);
			var year = Tubi.GetYear(pageHtml, startPositionMediaBlock, title);
			...
			var mediaURL = Tubi.GetMediaURL(pageHtml, startPositionMediaBlock, title, year);
			var type = Tubi.GetMediaType(mediaURL, title, year);
			...
			var (posterURL, startPosition) =
				Tubi.GetPosterURLAndStartPosition(pageHtml, startPositionMediaBlock, title, year);
			(var duration, startPosition) =
				Tubi.GetDurationAndUpdatedStartPosition(type, startPosition, pageHtml, title, year);
			(var ageRating, startPosition) =
				Tubi.GetAgeRatingAndUpdatedStartPosition(type, startPosition, pageHtml, title, year);
			var genre = Tubi.GetGenre(startPosition, pageHtml, title, year, categoryName);

			if (
				ageRating <= ViewModel.MaxAgeRating
			)
			{
				if (sourceURL.Contains("https://tubitv.com/search/"))
				{
	...
	public static string GetMediaURL(string pageHtml, int startPositionMediaBlock, string title, string year)
	{
		...
		var mediaURL = "https://tubitv.com" + mediaURLSuffix;
	...
	{
		return "https://tubitv.com/search/" + searchTerm;
		// Tubi search scrapes don't return any useful media information
		// return null;
	}
	...
	private static (string?, int) GetPosterURLAndStartPosition(string pageHtml, int startPositionMediaBlock, string title, string year)
	{
		...
		var startPosition = pageHtml.IndexOf("//canvas-lb.tubitv.com/", startPositionMediaBlock);
		int endPosition;
		if (startPosition != -1)
		{
